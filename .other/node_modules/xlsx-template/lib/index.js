...
//			if (views) self.workbook.remove(views);
    ...
    */
    ...
    //must point to correct <Relationship> 'Id' in xl\_rels\workbook.xml.rels
      ...
      .sort(function(rel1, rel2){ //using order
        ...
        if ((index1 + index2) === 0){ //compare rels to worksheet
					 return	rel1.attrib.Target > rel2.attrib.Target
				}
      ...
      self.workbook.findall("sheets/sheet")
			.forEach(function(item, index) {
    ...
    Workbook.prototype.rebuildCalcChainForSheet = function(sheet, calcChain) {
      ...
      sheetData.findall('row').forEach(function(row) {
        row.findall('c').forEach(function(cell) {
          var formulas = cell.findall('f');
          if (formulas && formulas.length > 0) {
            var c = etree.SubElement(calcChain,'c');
		...
		//todo: !!!!
		//	console.log(etree.tostring(calcChain));
...
// TODO: if we do substitution via "shared strings", it causes changes in all pages 
// * comment to avoid this there:
