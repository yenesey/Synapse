...
var path = require('path'),
	iconv = require('iconv-lite'), //перекодировка для wscript 
...
function exec (process, args, options, onData, onExit){
	return new Promise(function(resolve, reject){
		...
		if (typeof onData !== 'function' || typeof onExit !== 'function')
			reject(new Error('wrong parameters <onData>, <onExit>'));
	
		var ps = cp.spawn(process, args, options);
	
		function _log(tail){
			if (options.log) //можно запретить журналирование
				console.log(ps.pid + ',' + tail)
		}
...
function task(params, onData, onExit){
	var args = [],	 //аргументы для spawn
		options = {},
		runtime = '',
		argv = JSON.stringify(params);
