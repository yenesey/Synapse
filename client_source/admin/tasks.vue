    ...
    <v-text-field v-if="taskId && access" 
      ...
      <v-dialog v-model="dialog" lazy>
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn v-show="selectIcon" color="blue darken-2" @click.native="taskIcon = selectIcon; dialog = !dialog; setMeta();">Выбрать</v-btn>
...
//var db = require('synapse/sqlite')('db/synapse.db'); //основная БД приложения
export default {
	data: function(){
		return {
      taskId:null,
    ...
    this.selectIcon = this.taskIcon
  }, 
	methods: {
		labelMenu : function(item){
      ...
      if(event.meta) {
        self.taskMeta = JSON.parse(event.meta);
        self.taskMenu = (self.taskMeta.menu) ? self.taskMeta.menu : '';
  			pxhr({ method:'get', url: '/tasks/meta?object=' + self.taskMenu})
  			.then(function(res){
		...
		setMeta : function(){ 
			var self = this;
      (self.taskIcon) ? self.taskMeta.icon = self.taskIcon : delete self.taskMeta.icon;
      (self.taskMenu && self.taskMenuName) ? self.taskMeta.menu = self.taskMenu : delete self.taskMeta.menu;
			return pxhr({ method:'put', url: '/tasks/meta', 
				data:{
					objectId: self.taskId, 
					meta: (self.taskMeta) ? JSON.stringify(self.taskMeta) : ''
				} 
			})
			.catch(function(err) {console.log(err)})
		}
